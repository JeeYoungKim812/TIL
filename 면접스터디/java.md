...
- 제어의 역행은 컨트롤의 제어권을 프레임워크가 가지게 되어 어플리케이션의 느슨한 결합을 도모합니다.
...
- 인터페이스는 모든 메서드가 추상메소드인 경우를 말합니다. 추상 클래스보다 한 단계 더 추상화된 클래스라고 볼 수 있습니다.
- 인터페이스는 상속관계에 연연하지 않고 각 기능을 클래스에서 오버라이딩 할 수 있기 때문에 여러가지 형태로 구현할 수 있습니다.
- 인터페이스-다중상속
- 인터페이스는 구현 객체가 같은 동작을 한다는것을 보장하는 것이 목적입니다.

---

- 추상클래스는 추상 method로 이루어진 클래스입니다. 추상메소드 뿐만 아니라 일반메소드도 구현 가능합니다.
- 추상클래스를 상속받은 클래스는 추상클래스 내의 추상메서드를 모두 구현해야합니다.
- 추상클래스-단일상속
- 추상클래스는 상속을 받아서 기능을 확장시키는것이 목적입니다.
